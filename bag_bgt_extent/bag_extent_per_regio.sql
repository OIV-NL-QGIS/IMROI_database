...
BEGIN
	...
	INSERT INTO algemeen.bag_extent (identificatie, status, geovlak, gebruiksdoel, bron, bron_tbl)
	SELECT o.gml_id, o.bgt_status, ST_Multi(ST_CurveToLine(o.geometrie_vlak))::geometry(MultiPolygon,28992), 'overigbouwwerk', 'BGT', 'Overigbouwwerk' FROM algemeen.veiligheidsregio_watergrens v
		LEFT JOIN overigbouwwerkactueel o ON ST_3DINTERSECTS(v.geom, o.geometrie_vlak)
	WHERE v.code = regio;

	INSERT INTO algemeen.bag_extent (identificatie, status, geovlak, gebruiksdoel, bron, bron_tbl)
	SELECT o.gml_id, o.bgt_status, ST_Multi(ST_CurveToLine(o.geometrie_vlak))::geometry(MultiPolygon,28992), 'tunneldeel', 'BGT', 'Tunneldeel' FROM algemeen.veiligheidsregio_watergrens v
		LEFT JOIN tunneldeelactueel o ON ST_3DINTERSECTS(v.geom, o.geometrie_vlak)
	WHERE v.code = regio;
