...
AS SELECT row_number() OVER () AS fid,
   ...
   FROM ( SELECT w.id,
            ...
            w.rotatie,
		    CASE
		        WHEN w.formaat_object = 'klein' THEN vt.size_bouwlaag_klein
		        WHEN w.formaat_object = 'middel' THEN vt.size_bouwlaag_middel
		        WHEN w.formaat_object = 'groot' THEN vt.size_bouwlaag_groot
		        WHEN w.formaat_bouwlaag = 'klein' THEN vt.size_object_klein
		        WHEN w.formaat_bouwlaag = 'middel' THEN vt.size_object_middel
		        WHEN w.formaat_bouwlaag = 'groot' THEN vt.size_object_groot
		    END AS size,
            w.symbol_name,
            w.bouwlaag,
            ''::text AS bouwlaag_object,
            'werkvoorraad'::text AS bron,
            w.datum_aangemaakt,
            w.datum_gewijzigd,
            CASE 
...
DROP FUNCTION IF EXISTS mobiel.funct_label_upd;
...
UPDATE algemeen.applicatie SET revisie = 10;
UPDATE algemeen.applicatie SET db_versie = 3610; -- db versie == versie_sub_revisie
